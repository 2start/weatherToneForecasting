...
object AvgNumMentionsJob {
  ...
  def main(args: Array[String]): Unit = {
    ...
    case class CEvent(id: Integer, date: Instant, numMentions: Integer, clz: String)
    ...
    eventNearestStations.join(maxTempWeather)
      ...
      .map(ev => {
        ...
        CEvent (ev.id, ev.date, ev.numMentions, c)
      })
      .keyBy(_.clz)
      .window(aggregateWindow)
      .apply { (key: String, window, events, out: Collector[(Instant, String, Double)]) =>
        val avg = events.map(_.numMentions.toDouble).sum / events.size
        out.collect((Instant.ofEpochMilli(window.getStart), key, avg))
      }
      .name("AVG NumMentions")
      //      .print()
      .writeAsCsv("avg-num-mentions.csv", WriteMode.OVERWRITE).setParallelism(1)
